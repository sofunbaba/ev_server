...
static bool verify_pkg(char *pkg, size_t len)
{
    ...
    ev_uint32_t head, session, cmd;
    ev_int32_t arglen;
    ev_uint16_t pkglen;

    assert(pkg != NULL);

    if(pkg_parse_data(pkg, &head, PKG_FIELD_HEAD) == false)
        return false;
    ret &= (head==DEFAULT_CMD_HEAD);

    if(pkg_parse_data(pkg, &session, PKG_FIELD_SESSION) == false)
        return false;

    if(pkg_parse_data(pkg, &pkglen, PKG_FIELD_PKGLEN) == false)
        return false;
    ret &= (pkglen==len);

    if(pkg_parse_data(pkg, &cmd, PKG_FIELD_CMD) == false)
        return false;
    if(pkg_parse_data(pkg, &opt, PKG_FIELD_OPT) == false)
        return false;
...
ev_int32_t pkg_parse_data(char *pkg, void *field, pkg_field_type_t type)
{
    ...
    if(field != NULL)
    {
        ...
        {
            if(hex2bin((ev_uint8_t *)field, pkg+pkg_field_data[type].position, pkg_field_data[type].size) == false)
                return false;
