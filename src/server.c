...
static ev_uint16_t opt_port = DEFAULT_SERVER_PORT;
...
static void opt_print_version()
{
    printf("%s, version:%s\r\n", DEFAULT_PROGRAM_NAME, DEFAULT_SERVER_VERSION);
    printf("   %s compiled %s\r\n\r\n", DEFAULT_COMPILE_OWNER, DEFAULT_COMPILE_DATE);
    exit(1);
}

/* These options are available from commandline */
static struct opt_table opt_cmdline_table[] = {
    OPT_WITHOUT_ARG("--version|-v", opt_print_version, NULL, "Print current version."),

    OPT_WITHOUT_ARG("--help|-h", opt_usage_and_exit,  NULL, "Print this message"),
...
static void opt_parse_table_init(int argc, char *argv[])
{
...
int main(int argc, char *argv[])
{
     ...
     * parse the cmd args.
     */
    opt_parse_table_init(argc, argv);

    if(opt_port < 1000 || opt_port > EV_UINT16_MAX)
    {
        log_msg(E_ERROR, "Invalid port!");
        return 1;
    }

    log_msg(E_INFO, "The server port: %d", opt_port);
